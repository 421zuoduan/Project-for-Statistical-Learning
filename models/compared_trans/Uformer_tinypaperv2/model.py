...
class WinReweight(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, win_size, kernel_size=3, stride=1, padding=1,
        ...
        self.ka_win_num = pow(int(input_resolution[0] // win_size), 2)
        self.num_heads = num_heads
        self.stride = stride
        ...
        self.downchannel = nn.Conv2d(dim * self.ka_win_num, self.ka_win_num, kernel_size=1, groups=self.ka_win_num)
        self.linear1 = nn.Conv2d(self.ka_win_num, self.ka_win_num * 4, kernel_size=1)
        self.gelu = nn.GELU()
        self.linear2 = nn.Conv2d(self.ka_win_num * 4, self.ka_win_num, kernel_size=1)
        self.softmax = nn.Softmax(dim=1)
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=8, shift_size=0,
        ...
        if min(self.input_resolution) >= self.window_size:
            self.spacial_se = Spacial_SELayer(channel=dim, win_size=window_size, inputsize=input_resolution)
    ...
    def forward(self, x, mask=None):
        ...
        if mask != None:
            ...
            input_mask_windows = window_partition(mask, self.window_size)  # nW, window_size, window_size, 1
        ...
        # partition windows
        x_windows = window_partition(shifted_x, self.window_size)  # B, nW*C, window_size, window_size
        ...
        # spacial se
        if min(self.input_resolution) > self.window_size:
            x = self.spacial_se(x.permute(0, 3, 1, 2)).permute(0, 3, 1, 2)
...
class Spacial_SELayer(nn.Module):
    def __init__(self, channel, win_size, inputsize, reduction=16, num_keep=4):
        super().__init__()
        self.channel = channel
        self.num_keep = num_keep
        self.win_size = win_size
        self.win_reweight = WinReweight(dim=channel, input_resolution=inputsize, num_heads=8, win_size=win_size)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.keep_feature_size = self.win_size * int(math.sqrt(self.num_keep))
        self.fc = nn.Sequential(
        nn.Linear(channel, channel//reduction, bias=False),
        nn.ReLU(inplace=True),
        nn.Linear(channel//reduction, channel, bias=False),
    ...
    def forward(self, x):
        ...
        # 权重生成
        # b, wn, 1, 1
        weight = self.win_reweight(x)
        sorted_weight, indices = torch.sort(weight, descending=True, dim=1)
        weight = torch.zeros_like(weight)
        weight.scatter_(1, indices[:, :self.num_keep, :, :], sorted_weight[:, :self.num_keep, :, :])
        nonzero_indices = weight.nonzero()[:, 1]  # 筛选层
        x_windowed = x.permute(0, 2, 3, 1).reshape(b, -1, self.channel)
        x_windowed = ka_window_partition(x_windowed, window_size=self.win_size).view(b, -1, self.channel, self.win_size, self.win_size)
        x_sel = torch.index_select(x_windowed, dim=1, index=nonzero_indices)[:, :4, :, :, :].view(b, -1, self.win_size, self.win_size)
        x_sel = ka_window_reverse(x_sel, self.win_size, self.keep_feature_size, self.keep_feature_size).reshape(b, self.keep_feature_size, self.keep_feature_size, self.channel).permute(0, 3, 1, 2)
        x_sel = self.avgpool(x_sel).view(b, c)

        y = self.fc(x_sel).view(b, c, 1, 1)
...
class Uformer(PatchMergeModule):
...
if __name__ == "__main__":
    ...
    # x = torch.randn(6, 3, input_size, input_size).cuda()
